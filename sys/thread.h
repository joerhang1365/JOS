...
struct thread_stack_anchor; // opague decl.
...
struct lock {
    ...
    struct thread * owner;      // pointer to thread that owns lock
    struct lock * next;         // next lock in lock list
    unsigned int cnt;           // number of times thread acquires lock
};
