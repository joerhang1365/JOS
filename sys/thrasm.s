...
# when new thread is called to resume for the first time it's return address
# points to _thread_startup

# thread_spawn set initial return address to _thread_start
# set thread context stack pointer
# passed entry function arguments to s0-7
# passed entry function pointer to s8

# looking at bottom half of _thread_swtch the new threads ctx is already
# loaded into sp, ra, and s0-11 registers

# set function's arguments passed in s0-7
...
# jump to entry function address and set return address to next instruction

        jalr    s8

# once thread reaches the end of it's function jump to thread_exit
