...
int syspipe(int * wfdptr, int * rfdptr)
{
	...
	create_pipe(&current_process()->iotab[wfd], &current_process()->iotab[rfd]);
...
int sysiodup(int oldfd, int newfd)
{
	...
	else if (oldfd < 0 || oldfd >= PROCESS_IOMAX || newfd >= PROCESS_IOMAX)
	{
		return -EBADFD;
	}
